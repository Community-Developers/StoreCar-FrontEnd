...
export class AdminService {
  ...
  public getDestaquesMobi(): Observable<Moto[]> {
    return this.http.get<{ content: Moto[] }>('http://18.220.168.177/motocicleta/destaques').pipe(
      map(res => res.content)
    );
  }
  ...
  postMoto(formData: FormData) {
    const headers = this.fetchData();
    return this.http.post('http://18.220.168.177/motocicleta', formData, { headers })
  }
  //===========POST SERVER ================
  ...
  public deleteImageCarro(id: string, nameObject: string): Observable<any> {
    const headers = this.fetchData();
    return this.http.delete(`http://18.220.168.177/veiculo/delete-images/${id}/${nameObject}`, { headers });
  }

  public deleteImageMoto(id: string, nameObject: string): Observable<any> {
    const headers = this.fetchData();
    return this.http.delete(`http://18.220.168.177/motocicleta/delete-images/${id}/${nameObject}`, { headers });
  }
  //=========== DELETE IMAGE SERVER ================
  ...
  public updateMobi(id: any, value: any): Observable<any> {
    return this.http.put(`http://18.220.168.177/motocicleta/${id}`, value, { headers: this.fetchData() });
  }

  public updateCarro(id: number, value: any): Observable<any> {
    return this.http.put(`http://18.220.168.177/veiculo/${id}`, value, { headers: this.fetchData() });
  }
  //================= UPDATE SERVER ============================
  ...
  saveNewImages(formData: FormData, type: string, id: string,): Observable<any> {
    return this.http.post(`http://18.220.168.177/${type}/save-images/${id}`, formData, { headers: this.fetchData() });
  }
  //================= SAVE NEW IMAGES ============================
